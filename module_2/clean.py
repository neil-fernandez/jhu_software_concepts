...
def clean_data(html):
    ...
    for row in soup.find_all("tr"):
        ...
        if len(columns) < 2:
            ...
            if term_match and not last_row.get("start_term"):
                last_row["start_term"] = term_match.group(0)
            ...
            if gre_general_match and not last_row.get("gre_general"):
                last_row["gre_general"] = gre_general_match.group(1)

            # get GRE V from regex match
            gre_v_match = re.search(
                r"\bGRE\s+V(?:erbal)?\s*[:=]?\s*(\d{2,3})\b", row_text, re.IGNORECASE
            )
            if gre_v_match and not last_row.get("gre_verbal"):
                last_row["gre_verbal"] = gre_v_match.group(1)
            ...
            for badge in badge_texts:
                ...
                if degree_match and not last_row.get("degree_level"):
                    token = degree_match.group(1).lower()
                    last_row["degree_level"] = (
                        "PhD" if "ph" in token or "doctor" in token else "Masters"
                    )
            if not last_row.get("degree_level"):
                degree_match = re.search(
                    r"\b(PhD|Ph\.D\.|Doctorate|MA|M\.A\.|MS|M\.S\.|MSc|Master(?:'s)?)\b",
                    row_text,
                    re.IGNORECASE,
